// Dendrites 1D 3-state cellular automata //
...
void loop() {
  ...
  for (int y = 0; y < HEIGHT; y++) {
    ...
    for (int x = 0; x < WIDTH; x++) {
      ...
      if (count == 0 || count == 4 || count == 6) child[x] = 0;
      if (count == 3 || count == 5) child[x] = 1;
      if (count == 1 || count == 2 ) child[x] = 2;
      if (trngGetRandomNumber()%rnd == 0) child[x] = 1;
