// Elevators 1D 3-state cellular automata //
...
void loop() {
  ...
  for (int y = 0; y < HEIGHT; y++) {
    ...
    for (int x = 0; x < WIDTH; x++) {
      ...
      if (count == 0 || count == 5 || count == 1) child[x] = 0;
      if (count == 3 || count == 4) child[x] = 1;
      if (count == 6 || count == 2 ) child[x] = 2;
      if (trngGetRandomNumber()%rnd == 0) child[x] = 0;
